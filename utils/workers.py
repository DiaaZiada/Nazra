...
def train(
    model, train_loader, valid_loader, n_epochs, optimizer, loss_function, model_file
):
    ...
    for e in range(1, n_epochs + 1):
        ...
        for x, y in train_loader:
            ...
            \nloss : {}\n\n".format(
                    ...
                    batch_number,
                    len(train_loader),
                    batch_number / len(train_loader) * 100.0,
                    delay * (len(train_loader) - batch_number),
                    delay,
                    loss.item(),
                )
            )
        ...
        with torch.no_grad():
            ...
            for x, y in valid_loader:
                ...
                \ndelay : {}s \nloss : {}\n\n".format(
                        ...
                        batch_number,
                        len(valid_loader),
                        batch_number / len(valid_loader) * 100.0,
                        delay * (len(valid_loader) - batch_number),
                        delay,
                        loss.item(),
                    )
                )
    ...
    with torch.no_grad():
        ...
        for x, y in test_loader:
            ...
            \ndelay : {}s \nloss : {}\n\n".format(
                    ...
                    batch_number,
                    len(test_loader),
                    batch_number / len(test_loader) * 100.0,
                    delay * (len(test_loader) - batch_number),
                    delay,
                    loss.item(),
                )
            )
